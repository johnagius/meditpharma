...
export function createApp({ document, window, pdfjsLib, XLSX }) {
  ...
  async function fetchFedexStatus(trackingNumber) {
    const base = getApiBase();
    if (!base) return { error: 'No Sync API URL set' };
    try {
      const res = await fetch(`${base}/api/track/${encodeURIComponent(trackingNumber)}`);
      const data = await res.json();
      return data; // always return — let caller check data.error
  ...
  async function syncAllStatuses(view) {
    ...
    // All rows with a tracking number (including already-delivered, to refresh date)
    const rows = view.getRows().filter((r) => !!(r.trackingNumber || '').trim());
    if (!rows.length) {
      if (typeof _toast === 'function') _toast('No rows with tracking numbers to sync.', 'warn');
      return;
    }

    // Find the active sync button (could be either tab)
    const syncBtn = document.getElementById('btn-trk-sync-all') || document.getElementById('btn-saved-trk-sync-all');
    const origLabel = syncBtn ? syncBtn.textContent : '';
    if (syncBtn) { syncBtn.textContent = '↺ Syncing…'; syncBtn.disabled = true; }
    const statusEl = view.status;
    setStatusInto(statusEl, `Syncing ${rows.length} rows from FedEx…`, 'ok');
    ...
    for (const row of rows) {
      ...
      if (!data || data.error) {
        failed += 1;
        if (!firstError) firstError = data ? data.error : 'fetch failed';
        continue;
      }
      if (!data.status || data.status === 'unknown') { failed += 1; continue; }

      const prevStatus = row.deliveryStatus;
      row.deliveryStatus = data.status;
      if (data.deliveredAtIso) {
      ...
      try { // eslint-disable-line no-await-in-loop
        ...
        if (!row.id && saved && saved.id) row.id = saved.id;
      } catch { /* non-fatal */ }
      if (row.deliveryStatus !== prevStatus) updated += 1;
    }

    if (syncBtn) { syncBtn.textContent = origLabel || '↺ Sync Statuses'; syncBtn.disabled = false; }

    const msg = `Sync done — ${updated} updated, ${rows.length - updated - failed} unchanged${failed ? `, ${failed} failed` : ''}${firstError ? ` (${firstError})` : ''}.`;
    setStatusInto(statusEl, msg, failed === rows.length ? 'warn' : 'ok');
    if (typeof _toast === 'function') _toast(msg, failed === rows.length ? 'err' : 'ok');
    renderAllTracking();
  }
  ...
  const COUNTRY_COLS = new Set(['country', 'manufacturingCountry']);
  ...
  function renderProductsTable() {
    ...
    rows.forEach((p) => {
      ...
      for (const [key, , cls] of PRODUCT_COLS) {
        const td = document.createElement('td');
        const inp = input(p[key], cls || 'w-md', (v) => {
          p[key] = v;
          if (COUNTRY_COLS.has(key)) inp.title = countryTitle(v);
        });
        if (COUNTRY_COLS.has(key)) inp.title = countryTitle(p[key]);
        td.appendChild(inp);
        tr.appendChild(td);
      }
